use crate::{
    ...
    error::Error,
    gsub::sets::{
        ChainSubruleSet, ChainedClassSeqRuleSet, LigatureSet, LookupRecord, Sequence, SubclassSet,
        SubruleSet,
    },
};
#[derive(Debug, Clone)]
...
pub enum ChainedContextualSubstitution {
    ...
    Format2 {
        ...
        chained_class_seq_rule_sets: Vec<ChainedClassSeqRuleSet>,
    },
...
impl ChainedContextualSubstitution {
    ...
    ///Blame the documentation for the long names.
    fn parse_format2(cursor: &mut Cursor, base: usize) -> Result<Self, Error> {
        ...
        let chained_class_seq_ruleset_offset_count = cursor.read_u16()?;
        let mut ruleset_offsets =
            Vec::with_capacity(chained_class_seq_ruleset_offset_count as usize);
        for _ in 0..chained_class_seq_ruleset_offset_count {
            ruleset_offsets.push(cursor.read_u16()?);
        }
        cursor.seek(coverage_offset as usize + base)?;
        let coverage = Coverage::parse(cursor)?;
        cursor.seek(backtrack_offset as usize + base)?;
        let backtrack_class = ClassDef::parse(cursor)?;
        cursor.seek(input_offset as usize + base)?;
        let input_class = ClassDef::parse(cursor)?;
        cursor.seek(lookahead_offset as usize + base)?;
        let lookahead_class = ClassDef::parse(cursor)?;
        let mut chained_class_seq_rule_sets =
            Vec::with_capacity(chained_class_seq_ruleset_offset_count as usize);
        for offset in ruleset_offsets {
            cursor.seek(start + offset as usize)?;
            chained_class_seq_rule_sets.push(ChainedClassSeqRuleSet::parse(cursor, base)?);
        }
        Ok(Self::Format2 {
            coverage,
            backtrack_class,
